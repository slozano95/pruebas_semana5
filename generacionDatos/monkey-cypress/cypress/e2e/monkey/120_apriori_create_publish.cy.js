...
import { login, clickOnButton } from "./_shared_slr";
...
describe("Testing Create publish meta data", () => {
    beforeEach(() => {
        cy.readFile('./mocks/119_post.json').then((fileData) => {
    ...
    it("Testing Create Post", () => {
        ...
        cy.visit(url).then((win) => {
            ...
            clickOnButton("Extra");
            cy.get('.post-setting-meta-description').type(DataPool.get("metaDescription"));               
            cy.wait(20);           
