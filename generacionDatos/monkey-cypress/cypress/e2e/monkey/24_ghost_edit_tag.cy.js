...
context('Editar tag with common data', () => {
  beforeEach(async() => {        
    try {
      ...
      metaDataTitle = DataPool.get("MetaDataTitle")
      metaDataDescription = DataPool.get("MetaDataDescriptionLong");
  ...
  describe('Edit tag ghost', () => {
    ...
    it('Login', () => {
      ...
      ss("SaveConfirmar")
      cy.wait(1000);      
      cy.get('p[class="response"]').should('contain', 'Meta Description cannot be longer than 500 characters.')
      cy.wait(1000);      
    })
  }) 
})